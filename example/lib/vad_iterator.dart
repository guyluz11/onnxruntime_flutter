...
class VadIterator {
  ...
  Future<bool> predict(Float32List data, bool concurrent) async {
    ...
    final output = (outputs?[0]?.getValue as List<List<double>>)[0][0];
    _hide = (outputs?[1]?.getValue as List<List<List<double>>>)
        .map((e) => e.map((e) => Float32List.fromList(e)).toList())
        .toList();
    _cell = (outputs?[2]?.getValue as List<List<List<double>>>)
        .map((e) => e.map((e) => Float32List.fromList(e)).toList())
        .toList();
